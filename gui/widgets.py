...
import gui as kex
from gui import KexWidget, restart_script, ping, pong
...
class ListBox(ScrollView):
    ...
    def _update(self, *a):
        ...
        if self.__last_list == new_list:
            return
        logger.info(f'ListBox found new list: {new_list} (last: {self.__last_list})')
        self.__last_list = new_list
        if callable(self.on_update):
            self.on_update(self.__last_list)
    ...
    def add_item(self, *a, text=None, do_update=True):
        logger.info(f'ListBox setting adding item: {text}')
...
class ListBoxItem(BoxLayout):
    def __init__(self,
            on_update, remove_callback, drag_callback,
        ...
        self.textinput = self.add(Entry(text=text, on_text=on_update))
